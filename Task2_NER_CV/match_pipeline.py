...
def parse_args():
    ...
    parser.add_argument(
        "--text", type=str, required=True, help="Text describing the image"
    )
    parser.add_argument(
...
def check_animal_match(text, image_path, dataset):
    ner_pipeline = pipeline(
        "ner", model="./animal_ner_model", tokenizer="./animal_ner_model"
    )
    entities = ner_pipeline(text)
    detected_animals = set(
        ent["word"].replace("##", "") for ent in entities if "LABEL" in ent["entity"]
    )
